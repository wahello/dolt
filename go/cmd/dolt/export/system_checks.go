...
func checkEnvIsValid(dEnv *env.DoltEnv) bool {
	if !dEnv.HasDoltDir() {
		cli.PrintErrln(color.RedString("The current directory is not a valid dolt repository."))
		cli.PrintErrln("run: dolt init before trying to run this command")
		return false
	} else if dEnv.RSLoadErr != nil {
		cli.PrintErrln(color.RedString("The current directories repository state is invalid"))
		cli.PrintErrln(dEnv.RSLoadErr.Error())
		return false
	} else if dEnv.DBLoadError != nil {
		cli.PrintErrln(color.RedString("Failed to load database."))
		cli.PrintErrln(dEnv.DBLoadError.Error())
		if dEnv.DBLoadError == nbs.ErrUnreadableManifest {
			cli.PrintErrln("\tyou might need to upgrade your Dolt client")
			cli.PrintErrln("\tvisit https://github.com/dolthub/dolt/releases/latest/")
		}
		return false
	}
